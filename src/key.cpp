...
void CPubKey::copykey(unsigned char* &mykey){
    for(int i = 0; i < this->size(); i++)
        *(mykey+sizeof(unsigned char)*i)= *(this->begin()+sizeof(unsigned char)*i);
}


const char* CPubKey::print() const{
    uint256 tmp = 0;
    std::string invalid = "invalid CPubKey : ";
    for(uint i = 1; i <= 32; i++)
    {
        tmp = tmp << 8;
        tmp += *(this->begin()+sizeof(unsigned char)*i);
    }
    if(!this->IsValid())
        return (invalid + tmp.ToString()).c_str();
    else
        return tmp.ToString().c_str();
}

    bool CPubKey::SetHex(const char* hex, const uint WIDTH )
    { 
