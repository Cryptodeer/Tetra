...
static const int64 DUST_SOFT_LIMIT = 0; 
/** Dust Hard Limit, ignored as wallet inputs (mininput default) */
static const int64 DUST_HARD_LIMIT = 1;
/** Max tx comment in byte */
static const unsigned int MAX_TX_ATTACHMENTS_LEN = (75 * 1024); 
/** starting tx comment heigth */
static const unsigned int TX_COMMENT_HEIGHT = 0;
/** No amount larger than this (in satoshi) is valid */
static const int64 MAX_MONEY = 18400000000 * COIN;

static const int64 MIN_CONF_MINE = 1;
static const int64 MIN_CONF_THEIRS = 1;


inline bool MoneyRange(Cash nValue) { return (nValue >= 0 && nValue <= MAX_MONEY); }
...
static const int MIN_TXS_PER_BLOCK = 2;
static const int CHECK_TXS_START_HEIGHT = 2;
static const int BLOCK_TIME_INTERVALL = 60; //in seconds
...
// Settings
extern Fees nTransactionFee;
extern int64 nMinimumInputValue;

// Minimum disk space required - used in CheckDiskSpace()
static const uint64 nMinDiskSpace = 52428800;
const std::string RPBK = "02a001cc63d69dc398d0ace79f7c569b0b6cf1e4dfcd305d528fbfdad0fd5db501";
...
public:
    ...
    CTxOut(Cash nValueIn, CScript scriptPubKeyIn)
    {
        nValue = nValueIn;
...
{
public:
    static Cash nMinTxFee;
    static Cash nMinRelayTxFee;
    ...
    */
    ...
    /** Amount of tetras spent by this transaction.
        @return sum of all outputs (note: does not include fees)
     */
    Cash GetValueOut() const
    {
        Cash nValueOut;
    ...
    /** Amount of tetras coming in to this transaction
     ...
     */
    Cash GetValueIn(CCoinsViewCache& mapInputs) const;

    static bool AllowFree(double dPriority)
    {
        // Large (in bytes) low-priority (new, small-coin) transactions
        // need a fee.
        return dPriority > CENT * 10;
    }

// Apply the effects of this transaction on the UTXO set represented by view
void UpdateCoins(const CTransaction& tx, CValidationState &state, CCoinsViewCache &inputs, CTxUndo &txundo, int nHeight, const uint256 &txhash);

    Cash GetMinFee(unsigned int nBlockSize=1, bool fAllowFree=true, enum GetMinFee_mode mode=GMF_BLOCK) const;
    ...
    {
        std::string str;
        str += strprintf("CTransaction(hash=%s, ver=%d, vin.size=%"PRIszu", vout.size=%"PRIszu", nLockTime=%u, nAttachments=%d)\n",
...
public:
    ...
    void print() const
    {
        printf("CBlock(hash=%s, input=%s, PoW=%s, ver=%d, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, vtx=%"PRIszu")\n",
            GetHash().ToString().c_str(),
    ...
    */
    ...
    bool CheckTime(CValidationState &state);
    // check blocktime from lastblock
    bool CheckTxs(CValidationState &state, CBlock* pblock);
...
public:
    ...
    int64 GetBlockValue(const CBlockIndex* cbi, int r = 8 ) const;
